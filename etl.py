...
# incidentes e acidentes e tabela de aeronaves envolvidas nas ocorrencias
# investigados pelo CENIPA e podem ser obtidos no endereço
# https://dados.gov.br/dataset/ocorrencias-aeronauticas-da-aviacao-civil-brasileira
# e irá gerar um arquivo Mysql criando os relacionamento entre tabelas, com os resultados
...
def abrir_arquivo_aero():
    try:
        ...
        valores_ausentes=['***','NULL']
        # Ao encontrar algo especificado em "valores_ausentes", estes serão automaticamente convertidos para Na ou Nan
        df_aero = pd.read_csv("aeronave.csv",sep=';',na_values=valores_ausentes)
...
def validar_arquivo_aero(df_aero):
    try:
        ...
        schema_aero.validate(df_aero,lazy=True)
        df_aero.drop_duplicates(['codigo_ocorrencia2'],inplace=True)
...
def exportar_mysql_aero(conexao,df_aero):
    try:
        # Converte dataframe para a tabela Mysql
        print('8 Gerando tabela aeronaves...')
        df_aero.to_sql('aeronaves', conexao, index=False)
